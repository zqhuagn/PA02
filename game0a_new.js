	...
	*/
	...
	function createMainScene(){
      ...
      edgeCam.position.set(20,20,10);
			...
			npc.addEventListener('collision',function(blue, relative_velocity, relative_rotation, contact_normal){
				if(blue==avatar){
				  gameState.health -=1;
				  this.__dirtyPosition = true;
				  this.position.set(randN(50),randN(50),randN(30));
				}
      })
			...
			obstacle = createConeMesh2(0xff0000,3,3);
			obstacle.position.set(10,5,4);
			obstacle.addEventListener('collision',function(redCone){
				if (redCone == avatar){
					gameState.score -= 1;
				}
			})
...
function addRedBalls(){
	...
	function addMedicineBall(){
	  ...
	  Mball.position.set(randN(20)+15,30,randN(20)+15);
	  scene.add(Mball);

	  Mball.addEventListener('collision',
	    function( Mball, relative_velocity, relative_rotation,contact_normal){
	      if(avatar==Mball){
	        gameState.health+=1;
	        this.position.y = this.position.y -100;
	        this.__dirtyPosition = true;
...
//addh
  ...
  function initPhysijs(){
	...
	*/
	...
	function createConeMesh2(color,r,h){
		var geometry = new THREE.ConeGeometry( r, h, 3);
		var material = new THREE.MeshLambertMaterial( { color: color} );
	  mesh = new Physijs.ConeMesh( geometry, material );
		mesh.castShadow = true;
		return mesh;
	}
  ...
  function createWall(color,w,h,d){
	...
	function keydown(event){
      ...
      case "3": gameState.camera = edgeCam; break;
			...
			// change left/right view of avatarCam
			case "q": avatarCam.rotateY(Math.PI/20);break;
			case "e": avatarCam.rotateY(-Math.PI/20);break;

			case ".": restorePosition(); break;
	...
	function restorePosition(){
		var v = THREE.Vector3(avatar.position.x,avatar.position.y,avatar.position.z);
		scene.remove(avatar);
		avatar = new createAvatar();
		scene.add(avatar);
		avatar.position.set(v);
	}
	...
	function updateNPC(){
	  ...
	  //npc.__dirtyPosition = true;
		var a = new THREE.Vector3(avatar.position.x,avatar.position.y,avatar.position.z);
		var n = new THREE.Vector3(npc.position.x,npc.position.y,npc.position.z);
		var distance=a.distanceTo(n);
		if (distance<=20){
